...
from ib_data_sdk.exceptions import (
    ConnectionError,
    DataRequestError,
    TimeoutError,
    ValidationError,
...
class TestIBDataClient:
    ...
    def test_get_historical_data_success(
        self, mock_ib_client, sample_historical_request, sample_trades_data
    ):
    ...
    def test_get_historical_data_timeout(
        self, mock_ib_client, sample_historical_request
    ):
    ...
    def test_get_historical_data_as_dataframe(
        self, mock_ib_client, sample_historical_request, sample_trades_data
    ):
    ...
    def test_error_handling(self, mock_ib_client):
        ...
        mock_ib_client.error(
            1, 2104, "Market data farm connection is OK"
        )  # Should not raise
        mock_ib_client.error(
            1, 321, "Market data farm connection is bad"
        )  # Should not raise
        mock_ib_client.error(
            1, 502, "Historical Market Data Service error"
        )  # Should not raise
